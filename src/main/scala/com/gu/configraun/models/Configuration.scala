...
case class Configuration(underlying: Map[String, Param]) {
  def getAsString(key: String): Either[ConfigraunError, String] = {
    underlying.get(key) match {
      ...
      case None => Left(ParamNotExistError(s"Parameter with key $key is not present.\n" +
        ...
        s"aws ssm put-parameter --name '/mystack/myapp/PROD$key' --value 'yourvalue' --type String", new UnsupportedOperationException))
  ...
  def getAsList(key: String): Either[ConfigraunError, Seq[String]] = {
    underlying.get(key) match {
      ...
      case None => Left(ParamNotExistError(s"Parameter with key $key is not present.\n" +
        s"Consider adding with " +
        s"aws ssm put-parameter --name '/mystack/myapp/PROD$key' --value 'yourvalue' --type StringList", new UnsupportedOperationException))
